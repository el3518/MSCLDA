...
def train(traepo,model):
    ...
    for i in range(1, args.iter + 1):#i=100
        ...
        domain_loss, class_loss, cls_loss, l1_loss, weight = model(source_data1, source_data2, target_data, source_label1, mark=1)
        ...
        domain_loss, class_loss, cls_loss, l1_loss, weight = model(source_data1, source_data2, target_data, source_label2, mark=2)
        ...
        if i % (args.log_interval * 10) == 0:
            t_num, t_accu = test(traepo, model, w1, w2)
...
def test(traepo, model, w1, w2):
    ...
    for data, target in target_test_loader:
        ...
        coml = preds2.eq(preds1.data.view_as(preds2)).cpu().sum()
        ...
        idmin = w.argmin()
        a = 24
        b = a / (2*preds1.shape[0])
        c = float(coml) / preds1.shape[0]
        if float(coml) >= a: 
            wmax = 1/(1 + math.exp(-(w[idmax] + (1-c)))) 
            wmin = 1/(1 + math.exp(-(w[idmin] - (1-c))))
            ws[idmax] = wmax/(wmax+wmin) + b
            ws[idmin] = wmin/(wmax+wmin) - b
            if ws[idmax] > 1:
                ws[idmax] = 1
                ws[idmin] = 0

        source_weight.append([w1, w2, ws[0].item(),ws[1].item(),float(coml)])
    ...
    correct_num = [correct1, correct2, correct, correctw, correctm]
    accu = [accu1, accu2, accu, accuw, accum]
    print(args.test_dir, '\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
            t_loss, correct, len(target_test_loader.dataset),
            100. * correct / len(target_test_loader.dataset)))
    print('\nsource1 {}, source2 {}, weight {} mean {}'.format(correct1, correct2, correctw, correctm))
